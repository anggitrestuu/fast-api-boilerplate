    ...
    async def delete(
        ...
        try:
            ...
            if self.has_soft_delete and not force:
                # Get the record first to handle unique fields
                if hasattr(self.model, "__unique_fields__"):
                    record = await self.get(id=id, filters=filters)
                    if record:
                        record.soft_delete  # This will update unique fields
                        await self.db.flush()
                        return True

                # Regular soft delete if no unique fields handling needed
                stmt = (
                    update(self.model)
